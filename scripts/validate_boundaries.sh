...
seed_review_files='examples/moontown_miniapp/projection_seed_review_moderation.mbt examples/moontown_miniapp/projection_seed_review_audit.mbt examples/moontown_miniapp/projection_seed_review_items.mbt examples/moontown_miniapp/projection_seed_review_runs.mbt examples/moontown_miniapp/projection_seed_review_tools.mbt'

seed_files="examples/moontown_miniapp/projection_seed_core.mbt examples/moontown_miniapp/projection_seed_review.mbt $seed_review_files examples/moontown_miniapp/projection_seed_content.mbt $seed_content_files examples/moontown_miniapp/projection_seed_shell.mbt $seed_shell_files examples/moontown_miniapp/projection_seed_attention.mbt examples/moontown_miniapp/projection_seed_backend.mbt $seed_backend_files examples/moontown_miniapp/projection_seed_places.mbt"
...
action_shell_files='examples/moontown_miniapp/projection_action_shell_navigation.mbt examples/moontown_miniapp/projection_action_shell_profile.mbt examples/moontown_miniapp/projection_action_shell_reviewer.mbt examples/moontown_miniapp/projection_action_shell_gates.mbt'
...
discover_market_files='examples/moontown_miniapp/discover_market_lists.mbt examples/moontown_miniapp/discover_market_entries.mbt examples/moontown_miniapp/discover_market_actions.mbt examples/moontown_miniapp/discover_market_primary_actions.mbt examples/moontown_miniapp/discover_market_buildings.mbt'
...
message_surface_files='examples/moontown_miniapp/message_attention.mbt examples/moontown_miniapp/message_channels.mbt examples/moontown_miniapp/message_context.mbt examples/moontown_miniapp/message_notices.mbt examples/moontown_miniapp/message_results.mbt examples/moontown_miniapp/message_reviews.mbt examples/moontown_miniapp/message_work.mbt'

message_attention_files='examples/moontown_miniapp/message_attention_model.mbt examples/moontown_miniapp/message_attention_panel.mbt examples/moontown_miniapp/message_attention_rows.mbt examples/moontown_miniapp/message_attention_reviews.mbt examples/moontown_miniapp/message_attention_work_rows.mbt examples/moontown_miniapp/message_attention_notices.mbt examples/moontown_miniapp/message_attention_render.mbt'
...
for required in $seed_review_files; do
  if [ ! -f "$required" ]; then
    printf '%s\n' "boundary violation: missing Moontown seeded review file $required"
    exit 1
  fi
done
...
for required in $action_shell_files; do
  if [ ! -f "$required" ]; then
    printf '%s\n' "boundary violation: missing Moontown shell action file $required"
    exit 1
  fi
done
...
for required in $demo_scene_files; do
  if [ ! -f "$required" ]; then
    printf '%s\n' "boundary violation: missing Moontown demo scene file $required"
    exit 1
  fi
done
...
seed_review_lines=$(wc -l < examples/moontown_miniapp/projection_seed_review.mbt | tr -d ' ')
if [ "$seed_review_lines" -gt 40 ]; then
  printf '%s\n' "boundary violation: projection_seed_review.mbt has $seed_review_lines lines; keep review seed data in focused projection_seed_review_* files"
  exit 1
fi
...
action_shell_lines=$(wc -l < examples/moontown_miniapp/projection_action_shell.mbt | tr -d ' ')
if [ "$action_shell_lines" -gt 40 ]; then
  printf '%s\n' "boundary violation: projection_action_shell.mbt has $action_shell_lines lines; keep shell action behavior in focused projection_action_shell_* files"
  exit 1
fi

agent_actions_lines=$(wc -l < examples/moontown_miniapp/projection_action_agents.mbt | tr -d ' ')
if [ "$agent_actions_lines" -gt 40 ]; then
  printf '%s\n' "boundary violation: projection_action_agents.mbt has $agent_actions_lines lines; keep agent action behavior in focused projection_action_agent_* files"
  exit 1
fi
...
demo_scene_lines=$(wc -l < examples/moontown_miniapp/demo_scene.mbt | tr -d ' ')
if [ "$demo_scene_lines" -gt 40 ]; then
  printf '%s\n' "boundary violation: demo_scene.mbt has $demo_scene_lines lines; keep scene behavior in focused demo_scene_* files"
  exit 1
fi
...
for focused_seed_review in $seed_review_files; do
  focused_lines=$(wc -l < "$focused_seed_review" | tr -d ' ')
  if [ "$focused_lines" -gt 120 ]; then
    printf '%s\n' "boundary violation: $focused_seed_review has $focused_lines lines; split the seeded review concern further"
    exit 1
  fi
done
...
for focused_action_shell in $action_shell_files; do
  focused_lines=$(wc -l < "$focused_action_shell" | tr -d ' ')
  if [ "$focused_lines" -gt 140 ]; then
    printf '%s\n' "boundary violation: $focused_action_shell has $focused_lines lines; split the shell action concern further"
    exit 1
  fi
done

for focused_agent_action in $agent_action_files; do
...
for focused_demo_scene in $demo_scene_files; do
  focused_lines=$(wc -l < "$focused_demo_scene" | tr -d ' ')
  if [ "$focused_lines" -gt 140 ]; then
    printf '%s\n' "boundary violation: $focused_demo_scene has $focused_lines lines; split the demo scene concern further"
    exit 1
  fi
done
